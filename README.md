        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
            <div class="border-b pb-3">
                ...
                <p class="text-slate-500 text-xs mt-1">
                    依據都市更新共同負擔比率評分標準，動態校正核心指標基準值 (N)
                </p>
            </div>
            
            <div class="text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100">
                本案核心指標為校正後的共同負擔比率基準值（計算至小數點後第 2 位，第 3 位四捨五入）。
                其計算邏輯係以<span class="font-bold text-slate-900">「容移30%之共同負擔比率 (n)」</span>為基礎，
                依據申請人承諾辦理之容積移轉值佔公告上限容移（30%）之比例，
                動態調校有容移與無容移之間的差值 (A)，藉此導出最符合專案現況之共同負擔比率。
            </div>

            <div class="bg-gradient-to-r from-slate-50 to-blue-50/30 p-5 rounded-xl border border-blue-100/50 text-center space-y-3">
                <span class="text-xs font-bold text-blue-800 uppercase tracking-wider block">【 核心動態試算公式 】</span>
                <div class="text-xl font-serif text-slate-800 my-2 overflow-x-auto py-1">
                    $$ N = n - \left( \frac{30 - T }{30} \times A \right) $$
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-2">
                <div class="p-3 bg-blue-50/40 rounded-xl border border-blue-100/50">
                    <span class="font-bold text-blue-900 text-sm block mb-1">N：基準值核心指標</span>
                    <p class="text-slate-600">本案校正後的共同負擔比率基準值（四捨五入至小數點後第2位）。</p>
                </div>
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/60">
                    <span class="font-bold text-slate-800 text-sm block mb-1">n：上限共負比</span>
                    <p class="text-slate-600">公告上限容積移轉（30%）時之共同負擔比率。</p>
                </div>
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/60">
                    <span class="font-bold text-slate-800 text-sm block mb-1">n'：無容移共負比</span>
                    <p class="text-slate-600">無容積移轉時之共同負擔比率。</p>
                </div>
                <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/60">
                    <span class="font-bold text-slate-800 text-sm block mb-1">A：共負比差值</span>
                    <p class="text-slate-600">公告有容移與無容移之共同負擔比率差值 (A = n - n')。</p>
                </div>
                <div class="p-3 bg-indigo-50/50 rounded-xl border border-indigo-100/50 md:col-span-2">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <span class="font-bold text-indigo-900 text-sm block mb-1">T：承諾容移值 (%)</span>
                            <p class="text-slate-600">申請人承諾之容積移轉值（系統強制限縮於 0% ~ 30% 之間）。</p>
                        </div>
                        <div>
                            <span class="font-bold text-emerald-900 text-sm block mb-1">R：承諾共負比 (%)</span>
                            <p class="text-slate-600">申請人投標承諾之共同負擔比率，用以與基準值 (N) 比對評分落點。</p>
                        </div>
                    </div>
                </div>
            </div>
